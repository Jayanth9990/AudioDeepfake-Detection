...
class My_SERes2Net_block(nn.Module):
    def __init__(
        ...
        self.conv1 = SEBottle2neck(
            inplanes=nb_filts[0], planes=nb_filts[1], kernel_size=(conv1[0], conv1[1])
        )
        self.selu = nn.SELU(inplace=True)
...
class SimpleRelativeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        relative_position_bias = self.relative_position_table[
            self.relative_index.reshape(-1)
        ].reshape(self.length, self.length, self.num_heads)
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()
        attn = attn + relative_position_bias.unsqueeze(0)
        attn = attn.softmax(-1)
        attn = self.attn_drop(attn)
        x = torch.einsum("bnqk,bnqe->bnqe", attn, v).transpose(1, 2).reshape(B, N, C)
...
class SE_Rawformer_front(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Sequential(
                My_SERes2Net_block(
                    nb_filts=filts[2], conv1=conv1, conv2=conv2, conv3=conv3
                )
            ),
            nn.Sequential(
                My_SERes2Net_block(
                    nb_filts=filts[3], conv1=conv1, conv2=conv2, conv3=conv3
                )
            ),
            nn.Sequential(
                My_SERes2Net_block(
                    nb_filts=filts[4], conv1=conv1, conv2=conv2, conv3=conv3
                )
            ),
        )
...
class SafeEar(nn.Module):
    def __init__(self, front, *args, **kwargs):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv1d(
                kwargs["embedding_dim"] * 7, kwargs["embedding_dim"], kernel_size=1
            ),
...
class SafeEar1s(nn.Module):
    ...
    def forward(self, encoder):
        ...
        # Temporal shuffle augmentation
        batch_size, feature_dim, frame_num = encoder.size()
        for i in range(0, frame_num, 50):
            end = min(i + 50, frame_num)
